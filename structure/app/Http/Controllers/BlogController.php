...
class BlogController extends Controller
{
	...
	public function index(){
		...
		return view("admin.configuraciones.index", array("blog"=>$blog));
          ...
          // "sobre_mi_completo" => 'required|regex:/^[(\\)\\=\\&\\$\\;\\-\\_\\*\\"\\<\\>\\?\\¿\\!\\¡\\:\\,\\.\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/i'
            ...
            if($logo["logo_temporal"] != ""){
                ...
                if($validarLogo->fails()){
                    ...
                    return redirect("/admin/configuraciones")->with("no-validacion-imagen", "");
    		...
    		if($validar->fails()){
    			...
    			return redirect("/admin/configuraciones")->with("no-validacion","");
    			...
    			return redirect("/admin/configuraciones")->with("ok-editar","");
    		}

    	}else{

    		return redirect("/admin/configuraciones")->with("error","");
