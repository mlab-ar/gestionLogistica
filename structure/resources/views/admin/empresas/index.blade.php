...
@section('content')
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Listado de Empresas habilitadas</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="/admin/dashboard">Home</a></li>
            <li class="breadcrumb-item active">Empresas habilitadas</li>
          </ol>
        </div>
      </div>
    </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="table-responsive">
              ...
              <tbody>
                @foreach ($empresas->where('id','!=',0) as $empresa)
                <tr>
                  <td> {{$empresa->name}} </td>
                  <td> {{$empresa->cant_max}} </td>
                  <td> {{$usuarios->where('last_name','=', $empresa->name)->count()}} </td>
                  <td> {{$empresa->cant_max - $usuarios->where('last_name','=', $empresa->name)->count() }} </td>
                  <td>
                    <a href="{{ route('admin.empresas.edit', $empresa )}}" class="btn btn-sm btn-primary"><i class="fas fa-user-edit"></i></a>
                    ...
                    <form method="POST"
                      ...
                      <button class="btn btn-sm btn-danger"
                        onclick="return confirm('Estás seguro de querer eliminar la empresa?')"
      ...
      <div class="modal-dialog">
        <div class="modal-content bg-secondary">
          <div class="modal-header">
            <h4 class="modal-title" id="myModal">Agrega el nombre de la empresa</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span></button>
          </div>
          <div class="modal-body">
            <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}" >
              <input class="form-control" name="name" value="{{old('name')}}" placeholder="Ingresa aquí el nombre de la empresa" required>
